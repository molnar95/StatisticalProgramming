...
###########################
# Többlépcsõs mintavétel  #
###########################

#2 lépcsõn haladunk végig. 
#Elõször kivesszük az osztályt (First Stage):
sample1 <- sample(unique(df$oszt_azonos_i), 10, replace = FALSE)
#9 elemû mintát vettünk a distinct osztályazonosítók közül.
sample1
unique(df$oszt_azonos_i)
#Ez egy vektor mely 9 elembõl áll. 
#Mi ezen 9 elemen/osztályon belüli magasság értékekre vagyunk kíváncsiak:
multidf <- df[df$oszt_azonos_i %in% sample1, ]
#Ebben a dataframe-be már csak az elsõ lépcsõ során kivett 9 véletlenszerûen
#kiválasztott osztály elemei lesznek.
multidf
#A második lépésben (Second Stage) 50 elemû véletlenmintát veszek ki a 9 random 
#osztályon belül:
...
#Elõször random mintavételezünk 9 osztályt a sokaságból:
